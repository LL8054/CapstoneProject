...
NextWordPredictor3 <- function(input=char()){
    ...
    if (n == 1) {
        grams <- readRDS("~/datasciencecoursera/Courses/Capstone/Subsets/bigramsDT25.rds")
        gramsFreq <- grams[order(count, decreasing=TRUE)]
        }
    if (n == 2) {
        grams <- readRDS("~/datasciencecoursera/Courses/Capstone/Subsets/trigramsDT25.rds")
        gramsFreq <- grams[order(count, decreasing=TRUE)]
        }
    if (n == 3) {
